...
def copy_data(file_name): # функция для копирования словаря в новый файл
    data = read_file(file_name)
    new_file_name = 'phone_copy.csv'
    standart_write(new_file_name, data)
    print(f'Данные скопированы в {new_file_name}')

def edit_row(file_name): # функция для изменения полей в словаре
    res = read_file(file_name)
    search = int(input('Введите номер строки для редактирования: '))
    if search <= len(res):
        new_data = get_info()
        res[search - 1]['first_name'] = new_data[0]
        res[search - 1]['second_name'] = new_data[1]
        res[search - 1]['phone_number'] = new_data[2]
        standart_write(file_name, res)
    else:
        print('Введен неверный номер строки')
